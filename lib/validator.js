...
Validator.prototype._validateCommand = function (line_, commands_) {
	...
	if (command === undefined)
		throw new Error ('Unknown ' + this.currentChunk + ' command "' +
		                 commandName + '"');
	...
	for (var i = 0; i < command.params.length; i++) {
		...
		if (error !== undefined) {
			throw new Error ('Error processing param "' + paramName + '" in '+
	...
	if (argList.length !== 0)
		throw new Error ('Extraneous parameters: ' + line_);
...
Validator.prototype.validateLine = function (line_) {
	...
	if (line === '$CONFIG') {
		if (this.parsedConfig)
			throw new Error ('There may only be one CONFIG chunk.');

		if (this.parsedFrame)
			throw new Error ('The CONFIG chunk must be the first chunk.');
	...
	throw new Error ('Expected chunk declaration, found "' + line + '" instead.');
};
